...
export function validateStudentPayload(payload) {
  ...
  if (!validStudentAccessToken(accessToken)) {
    return { ok: false, status: 403, error: "Enter the class access code to save resources." };
  }
  ...
  if (website === null) return { ok: false, status: 400, error: "Website must start with http:// or https://." };
...
export function studentAccessRequired() {
  return Boolean(process.env.STUDENT_ACCESS_TOKEN);
}
...
export function validStudentAccessToken(value) {
  return !studentAccessRequired() || value === process.env.STUDENT_ACCESS_TOKEN;
}

export function validAdminAccessToken(value) {
  return !adminAccessRequired() || value === process.env.ADMIN_ACCESS_TOKEN;
}
...
export async function listPendingSupabaseResources(adminToken) {
  if (!supabaseConfigured()) {
    return { ok: false, status: 503, error: "Supabase is not configured." };
  }
  if (!validAdminAccessToken(String(adminToken || "").trim())) {
    return { ok: false, status: 403, error: "Enter the admin access code to review resources." };
  }
  ...
  if (!response.ok) {
    const error = await response.text();
    return { ok: false, status: 502, error: `Supabase review lookup failed: ${error || response.status}` };
  }
  ...
  return {
    ok: true,
    status: 200,
    resources: rows.map(supabaseRowToResource)
  };
}

export async function reviewSupabaseResource(payload) {
  if (!supabaseConfigured()) {
    return { ok: false, status: 503, error: "Supabase is not configured." };
  }
  if (!validAdminAccessToken(String(payload.adminToken || "").trim())) {
    return { ok: false, status: 403, error: "Enter the admin access code to review resources." };
  }
  ...
  if (!id) return { ok: false, status: 400, error: "Missing resource id." };
  if (!["approved", "rejected"].includes(decision)) {
    return { ok: false, status: 400, error: "Review status must be approved or rejected." };
  }
  ...
  if (!response.ok) {
    const error = await response.text();
    return { ok: false, status: 502, error: `Supabase review update failed: ${error || response.status}` };
  }
